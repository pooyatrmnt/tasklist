...
<form method="POST" action="{{ isset($task) ? route('tasks.update', ['task' => $task -> id]) : route('tasks.store') }}">
    ...
    <div style="mb-4">
        ...
        @class(["border-red-500" => $errors->has("description")]) rows="5">{{ $task->description ?? old("description") }}</textarea>
        @error("description")
        <p class="error"> {{ $message }} </p>
        @enderror
    </div>
    ...
    <div style="mb-4">
        ...
        @class(["border-red-500" => $errors->has("long_description")]) rows="10">{{ $task->long_description ?? old("long_description") }}</textarea>
